...
import { isExistingNumber } from "../../../helpers/ownerHelpers";
const OTPLogin = () => {
  ...
  const getOtp = async (e) => {
    ...
    try {
      ...
      if ( !extractedNumber ||extractedNumber.length < 10 || extractedNumber.length > 10) {
      ...
      console.log(valid,"kk");
        if (valid.status) {
            //response from firebase
            setOwnerId(valid.owner._id)
    ...
    try {
      ...
      if (!myotp) {
        ...
        await confirmObj.confirm(myotp).then((result) => {
          ...
          navigate(`/owner/new-password/${ownerId}`);
        });
      }
    } catch (error) {
        // User signup failed
