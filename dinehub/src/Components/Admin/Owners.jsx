...
{allOwnersData?.map((owner,index)=>{


    return(  <div key={index} className='grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5'>
        <div className='col-span-3 flex items-center'>
          <div className='flex flex-col gap-4 sm:flex-row sm:items-center'>
            <div className='h-12.5 w-15 rounded-md'>
              <img className='w-8 ' src='https://cdn-icons-png.flaticon.com/512/17/17004.png' alt='Product' />
            </div>
        ...
        <div className='col-span-1 flex items-center'>
          <p className='text-sm text-black dark:text-white'>{owner.phone}</p>
        </div>
        <div className='col-span-1 flex items-center'>
          <p className='text-sm text-black dark:text-white'>{owner.owner}</p>
        </div>
        <div className='col-span-1 my-5 flex px-10 items-center'>
       {owner.isBlocked?
       <button onClick={()=>handleUnBlock(owner._id)} className="rounded-md bg-red-600 px-3 py-1.5 text-base font-semibold leading-7 text-white hover:bg-red-500 ">
       Unblock
     </button>
        : 
       <button onClick={()=>handleBlock(owner._id)} className="rounded-md bg-green-600 px-3.5 py-1.5 text-base font-semibold leading-7 text-white hover:bg-green-500 ">
      Block
     </button>
       }
