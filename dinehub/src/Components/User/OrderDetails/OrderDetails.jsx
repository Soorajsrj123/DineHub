...
function OrderDetails() {
   ...
   let sum=0
    ...
    UserOrderDetails(user).then((data)=>{
       
      if(data){
        // SET USER IN IN STATE
        setResId(data.UserOrders[0].ownerId)
        // SET ORDERS IN STATE
    ...
    useEffect(()=>{
        getRestaurantDetails(resId).then((res)=>{
            if(res){
  ...
  return (
    ...
    return (    <div key={index} className="py-14 border-b-2 border-gray-400 px-4  md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
          ...
          <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch  w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
                    ...
                    <div className="w-full flex flex-col justify-start items-start space-y-8">
                      ...
                      <div className="flex justify-start items-start flex-col space-y-2">
                        <p className="text-sm leading-none text-gray-800">
                          <span className="text-gray-600">catagory: </span>{dish.category}
                        </p>
                        <p className="text-sm leading-none text-gray-800">
                          <span className="text-gray-600">price: </span> {dish.price}
                        </p>
                      </div>
                    </div>
                    <div className="flex justify-between space-x-8 items-start w-full">
                      <p className="text-base xl:text-lg leading-6">
                        <span className="text-red-500 ">price :{dish.price}</span>
                      </p>
            ...
            <div className="bg-gray-50 w-full xl:w-96 flex justify-between items-center md:items-start px-4 py-6 md:p-6 xl:p-8 flex-col ">
                    <h3 className="text-xl font-semibold leading-5 text-gray-800">Restaurant</h3>
                    <div className="flex  flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0 ">
                        <div className="flex flex-col justify-start items-start flex-shrink-0">
                            <div className="flex justify-center  w-full  md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                                <img className="w-32" src="https://upload.wikimedia.org/wikipedia/commons/6/62/Barbieri_-_ViaSophia25668.jpg" alt="avatar" />
                                <div className=" flex justify-start items-start flex-col space-y-2">
                            ...
                            <div className="flex justify-center  md:justify-start items-center space-x-4 py-4 border-b border-gray-200 w-full">
                                ...
                                <p className="cursor-pointer text-sm leading-5 text-gray-800">{restaurant?.email}</p>
                            </div>
                        </div>
                        <div className="flex justify-between xl:h-full  items-stretch w-full flex-col mt-6 md:mt-0">
                            <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row  items-center md:items-start ">
                                <div className="flex justify-center md:justify-start  items-center md:items-start flex-col space-y-4 xl:mt-8">
                                    ...
                                    <p className="w-48 lg:w-full xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">{restaurant.address}</p>
                                </div>
